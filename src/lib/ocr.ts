...
function parseListRows(lines: OcrLine[]): { trade: ParsedTrade; profitWord?: OcrWord }[] {
  /*
    ...
    以前はロットまで揃って初めて1件と数えていた。そのせいで
      XAUUSD.raw buy u.us 161      ← 「0.05」が「u.us」に化けた
      4433.24 → 4433.45 2026.09.08 07:39:01
    のような行が、まるごと消えていた。銘柄も損益も時刻も読めているのに、
    ロット1つのために何も出ないほうが困る。
    （実物のスクショで起きた。MT5 の浮いているタブの下に入って薄くなった行）

    出しておけば「入力が必要」と赤く出るので、消えたことに気づけるし、
    その場で打ち直せば登録できる。
  */
  const head = new RegExp(
    `([A-Z][A-Z0-9]{2,}(?:\\.[A-Za-z]+)?)\\s+(buy|sell)(?:\\s+(\\d+(?:\\.\\d+)?))?(?![\\w.])`,
    'i',
  )
  const tailNum = new RegExp(`(?:^|[^\\d.,])(${NUM})\\s*$`)
  const PRICE = String.raw`\d{2,7}\.\d{1,3}`
