...
import { deleteTrade, updateTradeNote } from '../lib/repo'
      ...
      {rows.map((t) => (
        <div key={t.id} className="card p-4">
          ...
          <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs sm:grid-cols-4">
            <Cell label="建値 → 決済" value={`${fmtNum(t.open_price)} → ${t.close_price != null ? fmtNum(t.close_price) : '—'}`} />
            ...
            {readOnly ? (
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2">
                  <button className="btn bg-accent text-white hover:bg-blue-500" onClick={() => saveNote(t.id)}>
                    保存
                  </button>
                  <button className="btn bg-panel-2 text-gray-300" onClick={() => setEditing(null)}>
                    キャンセル
                  </button>
                  <button className="btn ml-auto text-down hover:bg-down/10" onClick={() => remove(t.id)}>
                    取引を削除
                  </button>
                </div>
              </div>
